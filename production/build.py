...
SOCIAL_IMAGE='/media-assets/hero.webp'
ASSET_DIMENSIONS={
 'hero':(333,374),
 'nozzle':(405,688),
 'cctv':(393,388),
...
def photo(key,slot,eager=False):
 ...
 width,height=ASSET_DIMENSIONS[key]
 return f'<img src="/media-assets/{key}.webp" alt="{alt}，非公司工程紀錄" data-media="{slot}" width="{width}" height="{height}" loading="{"eager" if eager else "lazy"}" decoding="async"'+(' fetchpriority="high"' if eager else '')+'>'
...
def inject_home_head(path,public=False):
 ...
 if DATA.get('site_url'):
  ...
  image=DATA['site_url'].rstrip('/')+SOCIAL_IMAGE
